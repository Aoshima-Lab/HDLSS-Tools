...
def DBDA(train_X, train_y, test_X , option = False):
    ...
    for label in unique_labels:
        ...
        score = np.sum((test_X - mean_in_class)**2, axis=0) - (trace_cov_matrix / num_data_in_class)
        class_scores.append(score)
    
    predictions = unique_labels[np.argmin(class_scores, axis=0)]
    
    if option:
        print("DBDA function called. Set option=False to hide this information.")
