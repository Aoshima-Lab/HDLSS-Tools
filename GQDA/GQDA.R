...
GQDA <- function(train_X, train_y, test_X, option = FALSE) {
  data_dim <- dim(train_X)[1]
  ...
  for (k in 1:num_classes){
    ...
    score <- data_dim * colSums((test_X - mean_in_class)^2) / trace_cov_matrix + 
             data_dim * log(trace_cov_matrix) -
             data_dim / num_data_in_class
    class_scores[,k] <- score
  }
  predictions <- unique_labels[apply(class_scores, 1, which.min)]

  if (option) {
  cat("GQDA function called. Set option=FALSE to hide this information.\n")
